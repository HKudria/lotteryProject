...
class LotteryRepository
{
    ...
    function selectLotteryByToken(string $token)
    {
        $sql = "SELECT * FROM lottery WHERE token = :token";
...
UPDATE lottery SET active = true WHERE id = :id";
    ...
    {
        $sql = "SELECT id,token, box_count FROM lottery WHERE active = true";
    ...
    {
        $sql = "SELECT lottery.id as id, lottery.item_count as presents, lottery.description, lottery.box_count as boxes, lottery.active, lottery_present.lotterry_id as lotId, lottery_present.name, lottery_present.number, user_token.token, user_token.is_used as tokenUsed, user_token.nick, user_token.updated_at FROM lottery INNER JOIN lottery_present ON lottery.id = lottery_present.lotterry_id LEFT JOIN user_token ON lottery_present.token_id = user_token.id ORDER BY lottery.id DESC LIMIT $limit";
        $query = Connection::dbQuery($sql);
    ...
    {
        $sql = "UPDATE lottery SET active = false WHERE active = true";
        Connection::dbQuery($sql);
        return ['message' => "Lottery was disactivated"];
    }
}